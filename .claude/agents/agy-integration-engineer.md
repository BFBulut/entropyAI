...
- `src/entropy/core/pending.py` (**Faz 14, yeni**: tek bekleyen işler kuyruğu — `list(kind)`, `resolve(id, decision, note)`, sinyal `pending_changed`; türler `tool_permission`, `desk_change`, `rule_candidate`, `skill_candidate`; sözleşme `docs/ARCHITECTURE.md` §6.7), `src/entropy/platform/proc.py` (`popen_kwargs` — `src/entropy/**` içindeki HER alt süreç çağrısı bunu alır, AST kapısı ölçer; MCP onay sunucusu dâhil), `src/entropy/brain/system_prompt.py` + `context_builder.py` (sohbet sürekliliği sözleşmesi §6.5: sistem istemi sabit, bağlam kullanıcı mesajında)
...
- `src/entropy/agents/ephemeral.py` (**Faz 14-C, kod**: geçici ajan yaşam döngüsü `prepare → spawn → stream → report → memory → cleanup`; `agent.md` argv'de `--agents` ile taşınır, her koşuda yeni `--session-id` + `--no-session-persistence`, proje kökü `--add-dir`'e GİRMEZ, yeteneğe göre adım tavanı, tek bildirim, `workdir` silinir — ARCHITECTURE §6.4/§10.2)
- `src/entropy/core/permission_server.py` + `permission_mcp_main.py` (**Faz 14-B, kod**: bağımlılıksız stdio MCP onay sunucusu, tek araç `mcp__entropy__approve`; sonuç nesnesi YALNIZ tek `text` parçası + `isError` — `structuredContent` eklemek koşumu kırar; 15 dk karar tavanı; ARCHITECTURE §6.6)

## Çalışma belleğin
İşe başlamadan önce `docs/STATE.md` (varsa) ve `docs/reports` altındaki en son ilerleme raporunu oku; sözleşmeleri (bus sinyalleri, kwarg adları) oradan doğrula. Faz 14 (14-A…14-E) **kod oldu**; çalışan sözleşmeler `docs/ARCHITECTURE.md` §6.4, §6.4-D, §6.5, §6.6, §6.7, §8, §10.2 ve faz raporu `docs/reports/2026-09-11_Faz14_Ilerleme_Raporu_v0.12.0.md` (plan: `docs/reports/2026-09-11_Faz14_Analiz_ve_Plan.md`, karar [ADR-0010](../../docs/adr/ADR-0010-gecici-ajan-mimarisi-langgraph-alinmadi.md)). 14-F kapanışı yürürlükte: canlı S4/S5 ve tam süit/build doğrulaması açık.
