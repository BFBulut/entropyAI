...
**Tasarım (D).** Senin "karmaşık ve modern değil" yargın sayıyla doğrulandı. Tasarım sistemi yok, iki yarım sistem var (`CYBER_THEME` ve `READING_TOKENS` aynı rolleri farklı değerlerle dolduruyor): kod tabanında 99 farklı hex, 223 yerel `setStyleSheet`, 87 sabit boyut, 9 yazı boyutu, 18 font ailesi; yazıların %63'ü kalın. Zen'de tek ekranda 153 görünür denetim, üst çubukta 18 öğe, sol sekme çubuğunun 7 sekmesinden 1920 px'te 3'ü görünüyor; merkez sütunun %93'ü boş; aynı rapor bilgisi 8 yüzeyde tutarsız sayılarla. Chat'te 1280×800'de sohbet gövdesi pencerenin %17'si. Üç erişilebilirlik ihlali (klavye odağı 0 düğmede görünür; panel kenarlığı kontrastı 1,39:1; 23 denetim 24 px altında). "Modern görünmeme"nin kaynağı: saf tayf (S=%100) 8 vurgu rengi + tamamen emoji ikonografi (376 kullanım, 76 emoji) + BÜYÜK HARF panel başlıkları. **Kütüphane kararı:** kendi belirteç sistemimiz + QtAwesome (MIT) ikonlar; `qfluentwidgets` reddedildi (GPLv3 / ücretli ticari lisans), `qt-material`/`qdarktheme` reddedildi (sorun tema değil, sistem yokluğu). Öneri: 12 renk belirteci (WCAG oranları hesaplanmış), 4 tipografi kademesi + mono, 4 px ızgara, 3 yarıçap, 14 bileşen, çalışır `build_qss()`; bilgi mimarisi sadeleştirme (sekmeler → dikey gezinme, üst çubuk 18 → 4, rapor yüzeyi 8 → 2, komut paleti birincil gezinme; "Bildirimler" 6 tıklamadan 1'e); Entropy için `ui-design` yeteneği (`SKILL.md`: 7 değişmez, ölç → planla → uygula → kanıtla süreci, çalıştırılabilir ölçüm betiği, 9 kapı eşiği) ve 8 kalıcı tasarım testi (`test_no_hardcoded_hex`, `test_focus_rules_exist`, `test_zen_fits_1366`…). Desk zaten daha disiplinli (17 hex), son adıma bırakılır.
...
| **11-E Tasarım sistemi ve arayüz sadeleştirme** | D §6'daki 6 adım: (1) belirteç sistemi + `build_qss()` + QtAwesome ikon seti (emoji ikonlar kalkar) + 8 tasarım testi; (2) kabuk: üst çubuk 18 → 4 öğe, dikey gezinme, komut paleti birincil; (3) Zen panelleri: rapor yüzeyi 8 → 2, çekirdek üst çubuğa, terminal katlanır çekmece, splitter derinliği 4 → 1; (4) Chat: krom ≤ %25, sohbet gövdesi öncelikli; (5) erişilebilirlik kapıları (odak halkası, kontrast ≥ 3:1, hedef ≥ 24 px); (6) Desk'e aynı belirteçler; `ui-design` SKILL.md Entropy'nin yeteneklerine eklenir | ui, qa | 0 | `v0.9.4` |
...
6. **Tasarım kütüphanesi:** **kendi belirteç sistemimiz + QtAwesome (MIT) ikonlar**; `qfluentwidgets` lisans nedeniyle (GPLv3 / ücretli) reddedildi.
